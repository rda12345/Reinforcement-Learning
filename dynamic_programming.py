...
class DPAgent(object):
    ...
    def policy_improvement(self):
        ...
        for s in range(self.n_states):
            a = self.policy[s]
            value_funcs = np.array([self.evaluate_V(s,a) for a in range(self.n_actions)])
            self.policy[s] = np.argmax(value_funcs)
            if a != self.policy[s]:
                policy_stable = False
        if policy_stable:
    ...
    def value_iteration(self):
        ...
        while Delta > self.threshold:
            ...
            for s in range(self.n_states):
                ...
                for a in range(self.n_actions):
                    values.append(self.evaluate_V(s, a))
                self.V[s] = max(values)
        ...
        for s in range(self.n_states):
            value_funcs = []
            for a in range(self.n_actions):
                value_funcs.append(self.evaluate_V(s, a))
            self.policy[s] = np.argmax(np.array(value_funcs))    
